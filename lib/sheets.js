  ...
  pgBootstrapPromise = (async () => {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS films_stock (
        ...
        old_barcodes TEXT NOT NULL DEFAULT '',
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_by TEXT NOT NULL DEFAULT ''
      );
    ...
    await sql`
      ALTER TABLE films_stock
      ADD COLUMN IF NOT EXISTS old_barcodes TEXT NOT NULL DEFAULT '';
    `;
...
function normalizeFilm(row) {
  return {
    ...
    old_barcodes: String(row.old_barcodes || "").trim(),
    created_at: toIso(row.created_at),
    updated_at: toIso(row.updated_at),
...
async function listFilms({ search = "", barcode = "", cellNo = "", limit = 500, offset = 0 } = {}) {
  ...
  const { rows } = await sql`
    SELECT id, name, barcode, cell_no, old_barcodes, created_at, updated_at, updated_by
    FROM films_stock
    WHERE (${String(search || "").trim() === ""} OR lower(name) LIKE ${searchLike} OR lower(barcode) LIKE ${searchLike} OR lower(cell_no) LIKE ${searchLike} OR lower(old_barcodes) LIKE ${searchLike})
      AND (${String(barcode || "").trim() === ""} OR lower(barcode) LIKE ${barcodeLike} OR lower(old_barcodes) LIKE ${barcodeLike})
...
async function upsertFilm(film) {
  ...
  const hasOldBarcodes =
    Object.prototype.hasOwnProperty.call(film, "old_barcodes") ||
    Object.prototype.hasOwnProperty.call(film, "oldBarcodes");
  const oldBarcodes = hasOldBarcodes
    ? normalizeOldBarcodesText(film.old_barcodes ?? film.oldBarcodes ?? "", barcode)
    : "";
  if (!name) throw new Error("Film name is required");
  if (!barcode) throw new Error("Film barcode is required");

  await sql`
    INSERT INTO films_stock (name, barcode, cell_no, old_barcodes, created_at, updated_at, updated_by)
    VALUES (
      ${name},
      ${barcode},
      ${cellNo},
    ...
    DO UPDATE SET
      name = EXCLUDED.name,
      old_barcodes = CASE WHEN ${hasOldBarcodes} THEN EXCLUDED.old_barcodes ELSE films_stock.old_barcodes END,
      updated_at = EXCLUDED.updated_at,
      updated_by = EXCLUDED.updated_by
  `;
...
async function findFilmsByBarcode(barcode) {
  ...
  const { rows } = await sql`
    ...
    WHERE barcode = ${b}
      OR EXISTS (
        SELECT 1
        FROM regexp_split_to_table(coalesce(films_stock.old_barcodes, ''), '[,;[:space:]]+') AS old_code(code)
        WHERE old_code.code = ${b}
      )
    ORDER BY CASE WHEN barcode = ${b} THEN 0 ELSE 1 END, cell_no ASC
  `;
  return rows.map(normalizeFilm);
}

async function updateFilmBaseByBarcode(film) {
  await ensurePgSchema();
  const name = String(film.name || "").trim();
  const barcode = String(film.barcode || "").trim();
  const oldBarcodes = normalizeOldBarcodesText(film.old_barcodes ?? film.oldBarcodes ?? "", barcode);
  const updatedBy = String(film.updated_by || film.updatedBy || "").trim();
  if (!name) throw new Error("Film name is required");
  if (!barcode) throw new Error("Film barcode is required");

  const candidates = [barcode, ...oldBarcodeTokens(oldBarcodes)];
  const matchesByKey = new Map();
  for (const code of candidates) {
    const matches = await findFilmsByBarcode(code);
    matches.forEach((row) => {
      const key = `${String(row.barcode || "").trim()}@@${String(row.cell_no || "").trim()}`;
      matchesByKey.set(key, row);
    });
  }

  if (!matchesByKey.size && name) {
    const { rows } = await sql`
      SELECT id, name, barcode, cell_no, old_barcodes, created_at, updated_at, updated_by
      FROM films_stock
      WHERE lower(name) = lower(${name})
      ORDER BY cell_no ASC, id ASC
    `;
    rows.map(normalizeFilm).forEach((row) => {
      const key = `${String(row.barcode || "").trim()}@@${String(row.cell_no || "").trim()}`;
      matchesByKey.set(key, row);
    });
  }
  ...
  if (!matches.length) {
    const created = await upsertFilm({
      name,
      barcode,
      cell_no: "",
      old_barcodes: oldBarcodes,
      updated_by: updatedBy,
    });
    return { created: created ? 1 : 0, updated: created ? 1 : 0, matched: 0 };
  }

  for (const row of matches) {
    const previousBarcode = String(row.barcode || "").trim();
    const cellNo = String(row.cell_no || "").trim();
    const mergedOldBarcodes = normalizeOldBarcodesText(
      [
        row.old_barcodes,
        oldBarcodes,
        previousBarcode && previousBarcode !== barcode ? previousBarcode : "",
      ].filter(Boolean).join(", "),
      barcode
    );
    await upsertFilm({
      ...
      barcode,
      cell_no: cellNo,
      old_barcodes: mergedOldBarcodes,
      updated_by: updatedBy,
    });
    if (previousBarcode && previousBarcode !== barcode) {
      await deleteFilmByBarcodeCell(previousBarcode, cellNo);
    }
  }

  return { created: 0, updated: matches.length, matched: matches.length };
}
