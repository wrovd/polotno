...
async function upsertBoxTrackingEntry(entry = {}) {
  ...
  await sql`
    INSERT INTO box_tracking_entries (
      box_code, location, product_barcode, product_name, created_at, updated_at, updated_by
    ) VALUES (
      ${boxCode}, ${location}, ${productBarcode}, ${productName}, ${createdAt}, ${now}, ${updatedBy}
    )
...
async function listBoxTrackingEntries({ search = "", barcode = "", location = "", limit = 800, offset = 0 } = {}) {
  ...
  const { rows } = await sql`
    SELECT id, box_code, location, product_barcode, product_name, created_at, updated_at, updated_by
...
async function removeBoxTrackingByBoxCode(boxCode) {
  ...
  const { rows } = await sql`
    ...
    WHERE box_code = ${code}
    RETURNING id, box_code, location, product_barcode, product_name, created_at, updated_at, updated_by
  `;
  return {
